 ...
 * <p>
 * 预防注入式SQL攻击
 * 使用预编译命令，好处：组织注入式攻击、效率高、占位符不能使用多值、预编译、占位符？下标从1开始 参数化sql查询
 */
public class TemplateJDBC3 {
    private Connection connection = null;
    private PreparedStatement statement=null;
    private ResultSet resultSet = null;
    private static final List<MemoGroup> list=new ArrayList<>();
...
//                handle方法入参为结果集，返回值为list
                List<MemoGroup> list=  this.collectResult(resultSet);
                return  handle.handle((R) list);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }else{
